...
exports.updatePost = asyncHandler(async (req, res, next) => {
  ...
  if (!updatePost) {
    return next(
      new errorResponse(`Post not found with id ${req.params.id}`, 401)
    );
  }
...
exports.like = asyncHandler(async (req, res, next) => {
  ...
  res.status(200).send({
    success: true,
    data: { like: details.likes.length, likes: details.likes },
  });
});
...
exports.editComment = asyncHandler(async (req, res, next) => {
  ...
  const commentIndex = comment.content.findIndex(
  ...
  if (commentIndex == -1) {
    next(
      new errorResponse(
        `No comments found with id ${req.params.commentId}`,
        401
      )
    );
  }
  ...
  if (
    comment.content[commentIndex].user != req.user.id &&
    req.user.role !== "admin"
  ) {
...
exports.deleteComment = asyncHandler(async (req, res, next) => {
  ...
  if (!comment) {
    return next(
      new errorResponse(`No post found with id ${req.params.id}`, 401)
    );
  }
...
exports.getAllPosts = asyncHandler(async (req, res, next) => {
  ...
  if (req.user._id == req.params.id) {
    ...
    return res
      .status(400)
      .send({ success: false, data: "not allowed to see user's post" });
  } else {
    if (!user.following.includes(req.params.id)) {
      return res
        .status(400)
        .send({ success: false, data: "not allowed to see user post" });
    }
    let posts = await Post.find({ user: req.params.id }).populate([
      { path: "comments", select: "content" },
      { path: "likes", select: "likes" },
  ...
  if (req.files.files.length) {
    for (let i = 0; i < req.files.files.length; i++) {
      ...
      if (file.size > "1000000000000000") {
        next(
          new errorResponse(
            `Please upload a video less than 1000000000000000`,
            401
          )
        );
      }
  ...
  for (let i = 0; i < post.videos.length; i++) {
    if (post.videos[i].public_id === req.body.video) {
      try {
        ...
        await post.save();
        await cloudinary.uploader.destroy(req.body.video, {
          resource_type: "video",
        });
      } catch (err) {
        console.log(err);
